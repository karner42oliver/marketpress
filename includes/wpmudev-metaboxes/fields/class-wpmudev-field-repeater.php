...
jQuery(document).ready(function($){
	...
	var initializeDragAndDrop = function() {
		...
		$('.wpmudev-subfields').not('.no-sorting').on('dragstart', '.wpmudev-subfield-group', function(e) {
			draggedElement = this;
			$(this).css('opacity', '0.5');
			e.originalEvent.dataTransfer.effectAllowed = 'move';
		}).on('dragend', '.wpmudev-subfield-group', function(e) {
			draggedElement = null;
			$(this).css('opacity', '1');
		}).on('dragover', '.wpmudev-subfield-group', function(e) {
			if (draggedElement && draggedElement !== this) {
				...
				updateOrdering($(this).closest('.wpmudev-subfields').find('.wpmudev-subfield-group'));
			}
		}).on('drop', '.wpmudev-subfield-group', function(e) {
			e.preventDefault();
			if (draggedElement) {
				$(document).trigger('wpmudev_repeater_field/stop_sort', [ $(draggedElement) ]);
			}
		});
		
		// Make group headers draggable
		$('.wpmudev-subfield-group-index').css('cursor', 'move');
		$('.wpmudev-subfield-group').attr('draggable', 'true');
	};
